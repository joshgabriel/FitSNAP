...
def errors(group, rtype, indices, df, weight_flag, covariance, fit_sam):
    ...
    rsq = np.round(1 - ssr / np.sum(np.square(this_true - (this_true / nconfig).sum())),4)
    error_record = {
        "Group": group,
    ...
    print ("Error Record",error_record['Group'],error_record['Subsystem'],\
            error_record['rmse'], error_record['rsq'])
...
#    self.errors.append(error_record)
    ...
    pred_mean = np.array(this_pred)
    pred_std = np.array(pf_std)
    np.save('PredMean_'+group+'_'+rtype+'_from_fit.npy',pred_mean)
    np.save('PredStd_'+group+'_'+rtype+'_from_fit.npy',pf_std)
    np.save('TrueWrtPred_'+group+'_'+rtype+'_from_fit.npy',this_true)
